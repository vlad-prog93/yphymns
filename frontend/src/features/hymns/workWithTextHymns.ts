...
export const parseHymnText = (text: string): ParsedPart[][] => {
  return text.split('\n').map((line) => {
    const regex = /\[(.*?)\]/g

    const parts: { word: string, chord: string | null }[] = []
    let lastIndex = 0
    let match
    ...
    while ((match = regex.exec(line)) !== null) {
      if (match.index > lastIndex) {
        parts.push({
          word: line.slice(lastIndex, match.index),
          chord: null,
        })
      }

      const inside = match[1]
      const chordMatch = inside.match(/\{([^}]+)\}/)

      if (chordMatch) {
        const chord = chordMatch[1]
        const word = inside.replace(/\{[^}]+\}/, '')

        parts.push({ word, chord })
      } else {
        parts.push({ word: inside, chord: null })
      }
